...
class PhotoOverlayService {
  ...
  void _drawMiniPolyline(
    ...
    final lngMetres =
        lngRange * 111000.0 * math.cos(midLat * math.pi / 180);
    ...
    canvas.drawCircle(
        toOffset(points.first), w * 0.010, Paint()..color = CupertinoColors.white);
  ...
  void _drawStats(Canvas canvas, RunSession session, double w, double h) {
    ...
    _drawText(canvas, '${session.formattedDistance} km', colW * 0,
        statY + labelSize * 1.6, colW, valueSize, CupertinoColors.white,
        bold: true);

    _drawText(
        canvas, 'PACE', colW * 1, statY, colW, labelSize, const Color(0x80FFFFFF));
    _drawText(canvas, '${session.formattedPace} /km', colW * 1,
        statY + labelSize * 1.6, colW, valueSize, CupertinoColors.white,
        bold: true);

    _drawText(
        canvas, 'TIME', colW * 2, statY, colW, labelSize, const Color(0x80FFFFFF));
